...
Manage Docker containers, images, volumes, networks, and Compose stacks using standard Docker CLI commands.


## Troubleshooting: Dependency Failures
If you see errors like `Container hermes-workspace-hermes-agent-1 Error dependency hermes-agent failed to start`, check:
1. **Host Port Conflicts:** Use `ss -tulpn | grep :PORT` to check if another process is holding the port. If so, kill the offending process (`fuser -k PORT/tcp`).
2. **Gateway Accessibility:** If `hermes-agent` fails to start/remain running, ensure it is configured to bind to `0.0.0.0` (in `docker-compose.yml`) if it needs to communicate with external containers.
3. **Healthcheck Failures:** If `healthcheck` fails, ensure the endpoint/path is correct (use `127.0.0.1` instead of `localhost` inside containers) or simplify the check (e.g., check for a file presence like `ls /path/to/file` if the endpoint is flaky).
4. **Volume Permissions:** Ensure the mapped volumes have proper permissions and are not being held by stale containers.
5. **Missing Config/Auth:** If the container exits with code 0 without errors in the logs, verify all required environment variables (passwords, API keys) are present in `.env`. The container may be shutting down because of missing mandatory configuration (e.g., `HERMES_PASSWORD` when exposing to non-loopback).
6. **Concurrent Process Conflict:** If you are running both a systemd service (e.g., `hermes-workspace.service`) and Docker Compose concurrently, they will conflict on ports (3000, 8642) and local PID files. **Avoid running both.**
   - Choose one management layer. If using `systemd` to manage the Workspace service, ensure it points the API URL to an existing gateway service (e.g., `HERMES_API_URL=http://127.0.0.1:8644` where 8644 is the gateway port) rather than spawning its own conflicting gateway instance via the workspace's built-in `pnpm dev` or `concurrently` scripts. Check `/etc/systemd/system/` for existing workspace services that may conflict with Docker Compose setups.

Use `docker compose ps -a` and `docker logs <container_name>` to diagnose why a service failed to remain running. Also check `docker compose config` to verify the final resolved configuration.
